...
let filterTypes = ['all', 'joinorleft', 'cmdexec', 'deathmsg', 'warn', 'error', 'info', 'chatmsg', 'other'];
let limit = 50;

function connectWebSocket() {
    socket = new WebSocket('ws://' + window.location.origin.replace('http://', '') + '/ws1' + '?limit=' + limit);

    const regexList = {
        chatRegex: /[0-9]{2}:[0-9]{2}:[0-9]{2}(\.[0-9]{1,3})?.*\[.*\].*<[\w\s\S]+>\s+[\w\s\S]+/i,
