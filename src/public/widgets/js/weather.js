    ...
    function fetchWeather() {
        fetch(`/weather?city=${city}&units=${units}&lang=${lang}`)
            ...
            .then(data => {
                ...
                forecastContainer.innerHTML = '<h3 ignoreScroll>5-Day-Forecast:</h3>';
                ...
                data.forecast.forEach((forecast, index) => {
                    ...
                    forecastItem.innerHTML = `
                        <div ignoreScroll class="timeline-point">${dayNumber}</div>
                        <div ignoreScroll class="forecast-details">
                             <p ignoreScroll>${dayString}</p>
                            <img ignoreScroll src="${forecast.iconUrl}" alt="Weather Icon" />
                            <span ignoreScroll>${forecast.temperature}°C, ${forecast.description}</span>
                        </div>
                    `;
