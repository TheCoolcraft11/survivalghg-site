function parseMarkdown(input) {
    ...
    input = input.replace(/\!\[([^\[]+)\]\((https?:\/\/[^\)]+)\)/g, '<img src="$2" alt="$1">');

    // 7. Convert links ([Link Text](URL))
    input = input.replace(/\[([^\[]+)\]\((https?:\/\/[^\)]+)\)/g, '<a href="$2">$1</a>');
...
function reverseMarkdown(input) {
    ...
    input = input.replace(/<div class="note">(.*?)<\/div>/g, '::note::$1::note::');

    input = input.replace(/<span class="highlight">(.*?)<\/span>/g, '==$1==');

    input = input.replace(/<a href="(https?:\/\/[^\"]+)">(.*?)<\/a>/g, '[$2]($1)');

    input = input.replace(/<img src="(https?:\/\/[^\"]+)" alt="([^"]+)">/g, '![$2]($1)');
